...
class notes_database():
    ...
    def Update_note(self,title,new_content,new_embedding):
        try:
            ...
            if exists:
                self.cur.execute("UPDATE notes SET content = ? , embedding = ? WHERE title = ?",(new_content,new_embedding,title,))
                self.save_changes()
    ...
    def search_note(self,title):
        # Partial match on title OR tags so "work" finds tagged notes too.
        pattern = f"%{title}%"
        try:
            self.cur.execute(
                "SELECT * FROM notes WHERE title LIKE ? OR tags LIKE ? ORDER BY creation_date DESC",
                (pattern, pattern),
            )
